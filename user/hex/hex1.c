...
edit_record er[MAX_EDIT];int erc = 0,ercmax;
...
#define    AREA_HEX         0
#define    AREA_ASCII       1
#define    COLOR_VIEW       (COLOR_PAIR(1))
#define    COLOR_EDIT       (COLOR_PAIR(2))
...
bool initial()
{
    ...
    init_pair(1,   COLOR_BLACK,   COLOR_GREEN); //浏览模式 绿底黑字
    init_pair(2,   COLOR_BLACK,   COLOR_RED);   //编辑模式 红底黑字
    init_pair(3,   COLOR_YELLOW,     COLOR_BLACK); //标记位置 黑底红字
...
bool filestat(char *filename)
{
    ...
    if (ret != -1)
    {
        if (S_ISLNK(info.st_mode)) {sprintf(tips,"%-49s", "Is a symbol link.");return 0;}
        if (S_ISREG(info.st_mode)) {sprintf(tips,"%-49s", "Is a normal file.");return 0;}
        if (S_ISDIR(info.st_mode)) {sprintf(tips,"%-49s", "Is a directory.");return -1;}
        if (S_ISCHR(info.st_mode)) {sprintf(tips,"%-49s", "Is a character device.");return -1;}
        if (S_ISBLK(info.st_mode)) {sprintf(tips,"%-49s", "Is a block device.");return -1;}
        sprintf(tips, "Is a unknow device.");
        return -1;
    }
    else
    {
...
void refrebuf()
{
    ...
    if ( rsz != 0)
    {
        ...
        for (i = 0; i<rsz; i++)
        {
            ...
            if(i > 7) mvaddstr(line, 10+i*3+1, prtbuf);
            else      mvaddstr(line, 10+i*3,   prtbuf);
            
            mvaddch(line, 59, '|');
            if (isprint(buf[i]))
            {
                mvaddch(line, 60+i, buf[i]);
            }
            else
            {
                mvaddch(line, 60+i, '.');
...
void setstatus()
{
    ...
    if (mode == MODE_VIEW || mode == MODE_VIEWONLY)
    {
        ...
        mvaddch(WORK_LINES-1, 57, shortcuts);
	mvaddch((fpt+ly*16+lx)*(WORK_LINES-3)/fsz+1, 76, ' ');
        mvaddch((fpt+ly*16+lx)*(WORK_LINES-3)/fsz+2, 76, ' ');
        mvaddstr(0,0,
